...
{
	char    **name_value = malloc(sizeof(char *) * 3);
	t_env_list *env;
	t_export *export_head;

	if (check_if_equal_is(str) && eqaul_last_only(str) == 0)// check equal amd after it  and eqaul only
	{
		name_value[0] = get_name(str);
		name_value[1] = get_value(str);
		if(check_if_exist(name_value[0],*export)) // if exixts
		{
				if(check_append(str))
				{
...
void    do_export(char *str[],t_export **export, t_env_list **env_list, int flag)
{
	int i;
	int j;
	char *temp;
